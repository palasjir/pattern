...
package org.patern.clustering.hierarchical.visualization;
...
import org.patern.clustering.hierarchical.ColorMap;

public class DendrogramPanel extends JPanel {

    private static final long serialVersionUID = 1L;

    private final static BasicStroke solidStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND);
    private final static BasicStroke cutStroke = new BasicStroke(2.5f, BasicStroke.CAP_BUTT, BasicStroke.CAP_BUTT);
    private static final Cursor defCursor = Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR);
    private static final Cursor hndCursor = Cursor.getPredefinedCursor(Cursor.E_RESIZE_CURSOR);
    ...
    private Color cutLineColor = Color.RED;
    ...
    private int cutValueDisplay;
    private boolean cursorInsideMoveArea = false;
    private boolean canDragLine = true;
    ...
    private void updateModelMetrics() {
        ...
        cutValue = maxX / 2.0;
        cutValueDisplay = toDisplay(cutValue);
    }
...
//                    String distanceValueStr = String.format("0.0" + scaleValueDecimals + "f", distanceValue);
                    String distanceValueStr = format.format(distanceValue);
            ...
            if(showCutLine){
                ...
                g2.drawLine(cutValueDisplay, yDisplayOrigin, cutValueDisplay, yDisplayOrigin + hDisplay);
            }
    ...
    private int toDisplay(double value){
        int wDisplay = getWidth() - borderLeft - borderRight;
        return (int) ((wModel - value) * wDisplay / wModel) - scalePadding - borderRight;
    }
    
    private double toValue(int value){
        System.out.println(value);
        int wDisplay = getWidth() - borderLeft - borderRight;
        return ((double) (getWidth() - scalePadding - borderRight - value ) * wModel / (double) wDisplay);
    }
    
    /**
     * Interaction by cutting.
     * 
     * @author palas.jiri@gmail.com
     */
    private final MouseAdapter mouseAdapter = new MouseAdapter() {
        ...
        public void mouseReleased(MouseEvent e) {
            cutValue = toValue(cutValueDisplay);
            
            List<Cluster> clusters = new ArrayList<>();
            model.makeCut(cutValue, clusters);
            model.setColorRecursively(lineColor);
            Collections.shuffle(clusters);
            
            ColorMap map = ColorMap.getJet(clusters.size());
            for (int i = 0; i < clusters.size(); i++) {
                Cluster cluster = clusters.get(i);
                Color color = map.table[i];
                cluster.setColorRecursively(color);
            }
        ...
        public void mouseMoved(MouseEvent e) {
            if(e.getX() >= cutValueDisplay - 10 && e.getX() <= cutValueDisplay + 10){
                if(!cursorInsideMoveArea){
                    cursorInsideMoveArea = true; 
                    setCursor(hndCursor);
                }
            }else{
                if(cursorInsideMoveArea){
                    cursorInsideMoveArea = false;
                    setCursor(defCursor);
                }
            }
        }
