...
public class PDistClusteringAlgorithm implements ClusteringAlgorithm {
    ...
    public Cluster performClustering(double[][] distances,
                                     Object[] objs, LinkageStrategy linkageStrategy) {

		/* Argument checks */
        if (distances == null || distances.length == 0) {
            throw new IllegalArgumentException("Invalid distance matrix");
        }
        if (distances[0].length != objs.length
                * (objs.length - 1) / 2) {
		...
		/* Setup model */
        List<Cluster> clusters = createClusters(objs);
    ...
    public Cluster performWeightedClustering(double[][] distances, Object[] objs,
        ...
        return performClustering(distances, objs, linkageStrategy);
    }
